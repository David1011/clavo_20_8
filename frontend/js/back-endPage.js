;(function($, window, document,undefined) {
  var cjjTable = function(ele,opt){
        ...
        this.defaults ={
            ...
            pageJson:{
                taskId:528710,
                pageSize:8,//ajax page size
                pageIndex:1,//ajax index
  ...
  cjjTable.prototype = {
        start:function(){
            ...
            var maxpagenumberBox = 5;//max page
            var histroy_DDBox = "";
            var histroy_DD = "";
            ...
            "<td  colspan='"+_this.options.title.length+"'>"+
                "<div style='float:right;margin-left:10px;' class='tfootRight'>"+
                    "<input  placeholder='Enter page number' type='text'>"+
                    "<button>Go</button>"+
                    "</div>"+
                    "<div style='float:right'>"+
                        "<span class='firstPage' style='margin-right:10px;cursor: pointer;float:left;display: none;margin-left: 10px;'>First</span>"+
                        "<span class='lastPage' style='margin-right:10px;cursor: pointer;float:left;display: none;'>Prev</span>"+
        ...
        page:function(Pagenumber, pageNUmber, maxpagenumberBox,json,that,e) {
            ...
            var maxpagenumberBoxBigbox = "";
            var maxpagenumberBoxBig = "";
            if (Math.ceil(json.total/ e.options.pageJson.pageSize) < maxpagenumberBox) {
                for (var i = 0; i < Math.ceil(json.total/ e.options.pageJson.pageSize); i++) {
                    var  className = "";
                    if(i==0){
                        className = "pagenumberBoxLi";
                    }
            ...
            if (Math.ceil(json.total/ pageNUmber) > maxpagenumberBox) {
                ...
                } else if (Pagenumber >= Math.ceil(maxpagenumberBox / 2) * 1 + 1 && Pagenumber < Math.ceil(json.total / pageNUmber) - Math.ceil(maxpagenumberBox / 2) + 2) {
                    maxpagenumberBoxBigbox = "";
                    maxpagenumberBoxBig = "";
                ...
                } else if (Pagenumber >= Math.ceil(json.total / pageNUmber) - Math.ceil(maxpagenumberBox / 2) + 2 && Pagenumber <= Math.ceil(json.total / pageNUmber)) {
                    maxpagenumberBoxBigbox = "";
                    maxpagenumberBoxBig = "";
                    for (var i = Math.ceil(json.total / pageNUmber) - maxpagenumberBox; i < Math.ceil(json.total / pageNUmber); i++) {
                        maxpagenumberBoxBig += '<li>' + (i * 1 + 1) + '</li>';
                        maxpagenumberBoxBigbox = maxpagenumberBoxBig;
                    }
                    $(that.selector+" .pagenumberBox").html(maxpagenumberBoxBigbox);
                    $(that.selector+' .pagenumberBox li').eq(Pagenumber - Math.ceil(json.total/ pageNUmber) + maxpagenumberBox * 1 - 1).addClass('pagenumberBoxLi');
                }
            } else {
                if (Pagenumber <= Math.ceil(json.total / pageNUmber)) {
            ...
            if(Math.ceil(json.total/e.options.pageJson.pageSize)<2){
